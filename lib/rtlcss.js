      ...
      if (prevent === false) {
        switch (node.type) {
          ...
          case 'decl':
            ...
            // clone the decl and push it into decls array
            var clonedDecl = node.clone()
            decls.push(clonedDecl)
            ...
            if(context.util.isLastOfType(node) && flipped > 0){
              ...
              // prefix the original one with .ltr if it's not started with .ltr
              if(node.parent.selector.trim().substring(0, 4) !== '.ltr') {
                node.parent.selector = '.ltr ' + node.parent.selector 
              }
            }
          ...
          case 'rule':
            ...
            flipped = 0
            // new rule, reset flipped decl array
            decls = []
